...
//#define USE_SPI_XMEGA
	...
	#define SOFT_SPI_MOSI_DIRSET()     DDRB |= (1<<PB3)
	#define SOFT_SPI_MISO_DIRSET()     DDRB &= ~(1<<PB4)
	#define SOFT_SPI_SCK_DIRSET()      DDRB |= (1<<PB5)
	#define SOFT_SPI_MISO_PULLUP_SET() PORTB |= (1<<PB4)

	#define SOFT_SPI_MOSI_HI()         PORTB |= (1<<PB3)
	#define SOFT_SPI_MOSI_LO()         PORTB &= ~(1<<PB3)

	#define SOFT_SPI_SCK_HI()          PORTB |= (1<<PB5)
	#define SOFT_SPI_SCK_LO()          PORTB &= ~(1<<PB5)

	#define SOFT_SPI_MISO_READ()       PINB & (1<<PB4)
#endif
